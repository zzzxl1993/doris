...
Status handle_match_pred(RuntimeState* state, const TabletSchemaSPtr& tablet_schema,
    ...
    auto* left_slot_ref = find_slot_ref(left);
    if (left_slot_ref == nullptr) {
        return Status::Error<ErrorCode::INVERTED_INDEX_NOT_SUPPORTED>(
                "MATCH_PRED left expr has no SlotRef");
    }
    auto* right_lit = static_cast<vectorized::VLiteral*>(expr->children()[1].get());

    const SlotDescriptor* sd = state->desc_tbl().get_slot_descriptor(left_slot_ref->slot_id());
    if (sd == nullptr) {
        return Status::Error<ErrorCode::INVERTED_INDEX_NOT_SUPPORTED>(
                "Could not resolve SlotDescriptor for slot_id={}", left_slot_ref->slot_id());
    }
    int32_t col_idx = tablet_schema->field_index(left_slot_ref->column_name());
    if (col_idx < 0) {
        return Status::Error<ErrorCode::INVERTED_INDEX_NOT_SUPPORTED>(
                "Cannot resolve column in TabletSchema for slot_id={}, name={}",
                left_slot_ref->slot_id(), left_slot_ref->column_name());
    }
