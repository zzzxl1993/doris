...
public class MultiMatch extends ScalarFunction
    ...
    public MultiMatch withChildren(List<Expression> children) {
        Preconditions.checkArgument(children.size() >= 3, "At least three arguments are required.");

        Preconditions.checkArgument(children.get(children.size() - 2).getDataType().isVarcharType(),
                "The second last argument must be of type VarcharType.");
        Preconditions.checkArgument(children.get(children.size() - 1).getDataType().isVarcharType(),
                "The last argument must be of type VarcharType.");

        for (int i = 0; i < children.size() - 2; i++) {
            Preconditions.checkArgument(
                    children.get(i).getDataType().isStringType()
                    || children.get(i).getDataType().isVarcharType(),
                    "All arguments before the last two must be of type StringType or VarcharType."
            );
        }
